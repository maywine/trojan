...
void UDPForwardSession::start()
{
    ...
    out_write_buf =
        TrojanRequest::generate(config.password.cbegin()->first, config.target_addr, config.target_port, false);
...
void UDPForwardSession::in_recv(const string& data)
{
    ...
    Log::log_with_endpoint(in_endpoint,
                           "sent a UDP packet of length " + to_string(length) + " bytes to " + config.target_addr + ':'
                               + to_string(config.target_port));
    sent_len += length;
    if (status == FORWARD)
    {
        status = FORWARDING;
