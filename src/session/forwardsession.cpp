...
tcp::socket& ForwardSession::accept_socket() { return in_socket; }
...
void ForwardSession::start()
{
    ...
    out_write_buf =
        TrojanRequest::generate(config.password.cbegin()->first, config.target_addr, config.target_port, true);
    in_async_read();
