...
{
    mysql_init(&con);
    Log::log_with_date_time("connecting to MySQL server " + config.mysql.server_addr + ':'
                                + to_string(config.mysql.server_port),
                            Log::INFO);
    if (!config.mysql.ca.empty())
    {
        ...
        {
            mysql_ssl_set(
                &con, config.mysql.key.c_str(), config.mysql.cert.c_str(), config.mysql.ca.c_str(), nullptr, nullptr);
        }
        else
        {
...
void Authenticator::record(const string& password, uint64_t download, uint64_t upload)
{
    ...
    if (mysql_query(&con,
                    ("UPDATE users SET download = download + " + to_string(download) + ", upload = upload + "
                     + to_string(upload) + " WHERE password = '" + password + '\'')
                        .c_str()))
    {
        Log::log_with_date_time(mysql_error(&con), Log::ERROR);
    }
...
Authenticator::~Authenticator() { mysql_close(&con); }
