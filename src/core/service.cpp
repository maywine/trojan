      ...
      udp_socket(io_context)
{
    ...
    ssl_context.set_options(context::default_workarounds | context::no_sslv2 | context::no_sslv3
    ...
    else
    {
            ...
            if (config.ssl.cert.empty())
            {
                ...
                if (status == noErr)
                {
                    ...
                    status            = SecKeychainSearchCreateFromAttributes(
                        pSecKeychain, kSecCertificateItemClass, NULL, &pSecKeychainSearch);
                    for (;;)
                    {
                        ...
                        if (status == noErr)
                        {
                            ...
                            status = SecKeychainItemCopyAttributesAndData(
                                pSecKeychainItem, NULL, NULL, NULL, &_pCertLength, &_pCertData);
        ...
        if (!config.ssl.alpn.empty())
        {
            SSL_CTX_set_alpn_protos(
                native_context, (unsigned char*)(config.ssl.alpn.c_str()), config.ssl.alpn.length());
        }
        if (config.ssl.reuse_session)
        {
...
void Service::run()
{
    ...
    Log::log_with_date_time(string("trojan service (") + rt + ") started at " + local_endpoint.address().to_string()
                                + ':' + to_string(local_endpoint.port()),
                            Log::WARN);
    io_context.run();
...
void Service::udp_async_read()
{
    udp_socket.async_receive_from(
        ...
        [this](const boost::system::error_code error, size_t length)
        {
            ...
            Log::log_with_endpoint(tcp::endpoint(udp_recv_endpoint.address(), udp_recv_endpoint.port()),
                                   "new UDP session");
            auto session = make_shared<UDPForwardSession>(
                ...
                [this](const udp::endpoint& endpoint, const string& data)
                {
                    ...
                    {
                        Log::log_with_endpoint(tcp::endpoint(endpoint.address(), endpoint.port()),
                                               "dropped a UDP packet due to firewall policy or rate limit");
                    }
                    else if (ec)
                    {
...
boost::asio::io_context& Service::service() { return io_context; }
